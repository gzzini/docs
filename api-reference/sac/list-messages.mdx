...
api: "GET https://sac-waffle-v2.theo-110.workers.dev/support/{id}/mensagens"
...
```bash Com Paginação
curl -X GET "https://sac-waffle-v2.theo-110.workers.dev/support/5016/mensagens?page=2&limit=10" \
  -H "Authorization: Bearer seu_token_jwt"
```

```bash Mais Recentes Primeiro
curl -X GET "https://sac-waffle-v2.theo-110.workers.dev/support/5016/mensagens?sort=desc&limit=5" \
  -H "Authorization: Bearer seu_token_jwt"
```
