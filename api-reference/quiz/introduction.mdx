...
description: "API para sistema de quiz de recuperação de streaks baseado nas edições diárias (matinal e noturna) do The News"
---

## Visão Geral

A API de Quiz fornece endpoints para o sistema de recuperação de streaks do The News. Os quizzes são gerados automaticamente a partir das edições diárias (matinal e noturna/NIGHT) da newsletter, permitindo que usuários recuperem seus streaks perdidos respondendo perguntas sobre o conteúdo.
...
O sistema gera quizzes automaticamente todos os dias às 7h UTC usando:

1. **Beehiiv API**: Busca a edição do dia anterior (matinal e noturna)
2. **OpenAI GPT-4o**: Gera 10 perguntas sobre o conteúdo de cada edição
3. **Cloudflare D1**: Armazena quizzes e tentativas

### Edições Suportadas

| Edição | Identificação no título | `quizDate` | `edition` |
|--------|------------------------|------------|-----------|
| Matinal | `DD/MM/YYYY ...` | `YYYY-MM-DD` | `morning` |
| Noturna | `NIGHT #NNN` | `YYYY-MM-DD-night` | `night` |

Cada dia pode gerar **dois quizzes**: um para a edição matinal e outro para a edição noturna (NIGHT).

<Warning>
  Aos domingos não há edição da newsletter, portanto o quiz de segunda-feira (matinal) é baseado na edição de sábado. Não há quiz noturno para domingos.
</Warning>
